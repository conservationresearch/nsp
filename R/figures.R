...
graph_benefit <- function(results_benefit_national, results_benefit_global, inputs){
  ...
  benefits_graph <- ggplot2::ggplot(sim_results_summary_benefits, ggplot2::aes(x = org_program, y = mean, fill = Category)) +
    ...
    ggplot2::labs(x = "Species Program",
                  #User will need to change ylab below according to analysis option used
  ...
  benefits_graph2 <- ggplot2::ggplot(sim_results_summary_benefits, ggplot2::aes(x = org_program, y = mean, fill = Category)) +
    ...
    ggplot2::geom_text(ggplot2::aes(label = lab), vjust=0.3, size = 10) +
    ggplot2::scale_fill_manual(values=c("gray30", "gray70")) +
    #ggplot2::ylim(0,max(sim_results_summary_benefits$P95)) +
    ggplot2::geom_errorbar(position=ggplot2::position_dodge(width=0.9), ggplot2::aes(ymin=P5, ymax=P95), width=.1, color = "black") +
    ggplot2::labs(x = "",
...
graph_cost <- function(results_cost_organization, results_cost_total, inputs){
  ...
  results_cost_total$Category <- "Financial Leverage" # "Total Project Cost"
  ...
  sim_results_summary_costs <- sim_results_summary_costs %>%
    dplyr::mutate(lab = dplyr::if_else(Category == "Financial Leverage" & org_program %in% conservation_breeding, "    B", ""))
  # remove the factor levels
  results_benefit_global$org_program <- as.character(results_benefit_global$org_program)
  ...
  costs_graph <- ggplot2::ggplot(sim_results_summary_costs, ggplot2::aes(x = org_program, y = mean/1000, alpha = Category)) +
    ...
    ggplot2::labs(x = "Program Species",
...
bargraph_binnedby_benefits<-function(results_benefit_national, results_benefit_global, inputs, results_overall){
...
bargraph_binnedby_currentgs<-function(results_benefit_global, results_benefit_national, inputs, results_overall){
  ...
  sim_results_summary_bincurrentgs$org_program <- factor(sim_results_summary_bincurrentgs$org_program, 
                                                         levels=unique(sim_results_summary_bincurrentgs$org_program[
                                                           order(-sim_results_summary_bincurrentgs$bin_numeric, -sim_results_summary_bincurrentgs$mean, decreasing=FALSE
                                                           )]),ordered=TRUE)
...
scatter_bin_currentgs_national<-function(results_overall,results_benefit_national){
  ...
  bin_currentgs_scatter_national<-ggplot2::ggplot(results_benefit_national,aes(x=mean_GScurrentNational, y=mean_cgain_national_org, label=org_program))+
    ...
    ggrepel::geom_text_repel(data=subset(results_benefit_national, mean_GScurrentNational<= 15 ),
                            box.padding=0.5, point.padding = 0.75, min.segment.length = 0.5, force=3, direction="both")+
...
scatter_bin_currentgs_global<-function(results_overall,results_benefit_global){
  ...
  bin_currentgs_scatter_global<-ggplot2::ggplot(results_benefit_global,aes(x=mean_GScurrentGlobal, y=mean_cgain_global_org, label=org_program))+
    ...
    ggplot2::geom_errorbar(position = ggplot2::position_dodge(width = 0.9), ggplot2::aes(ymin = P5, ymax = P95), width = 0.1, color = "black") +
    ggrepel::geom_text_repel(data=subset(results_benefit_global, bin_GScurrentGlobal=="Extinct in the Wild" | bin_GScurrentGlobal=="Critically Depleted"), force=2,label.padding=0.5)+
...
graph_BCR <- function(results_BCR_national,results_BCR_global){
  ...
  ##Make the figure
  BCR_graph <- ggplot2::ggplot(results_BCR_all, ggplot2::aes(x = org_program, y = mean*100000, alpha = Category)) +
    ggplot2::geom_bar(stat="identity",position = ggplot2::position_dodge()) +
    ggplot2::facet_wrap(. ~ extirpated_or_extinct, scales="free") + 
    ggplot2::scale_alpha_manual(values=c(1,.3)) +
    ...
    ggplot2::labs( x = "",
                  ...
                  y = "Average BCR \n (Green Score Benefit per 100 000 CAD)",
                  title = "A") +
    ...
    ggplot2::theme(axis.text.x=ggplot2::element_text(angle=60, hjust=1), legend.justification = c(1,1),legend.position = c(0.95,0.95),
                   panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank()) +
    ggplot2::geom_text(ggplot2::aes(label = lab), show.legend = FALSE, vjust=0.4, size = 10, color = "black") 
  ...
  # get the number of unique x-axis values per facet (1 & 3, in this case)
  x.var <- sapply(ggplot2::ggplot_build(BCR_graph)$layout$panel_scales_x,
                  function(l) length(l$range$range))
...
graph_BCR_uncertainty <- function(results_BCR_national,results_BCR_global, inputs){
  ...
  cred_intervals$org_program <- factor(cred_intervals$org_program, 
                                        levels=unique(cred_intervals$org_program[order(-cred_intervals$ci90, decreasing = FALSE)]), ordered=TRUE)
  #### Make figure 
  BCR_credible_intervals<-  ggplot2::ggplot(cred_intervals, ggplot2::aes(x = org_program, y = ci90, alpha = Category)) +
    ggplot2::geom_bar(stat="identity",position = ggplot2::position_dodge()) +
    ggplot2::facet_wrap(. ~ extirpated_or_extinct, scales="free") + 
    ggplot2::scale_alpha_manual(values=c(1,.3)) +
    ggplot2::labs(x = "Species Program",
  ...
  # change the relative widths of the facet columns based on
  # how many unique x-axis values are in each facet
  gp2$widths[facet.columns2] <- gp2$widths[facet.columns2] * x.var2
